...
// Sync Cloudflare DNS to match SERVER keys/records (Fix A)
export async function POST(req: NextRequest) {
  ...
  if (!tenantId) return NextResponse.json({ ok: false, error: "domain_not_linked_to_tenant" }, { status: 400 });

  const job = await enqueueJob("mailstack:dns-sync", { tenantId, domainId: d.id, domainName: d.name });
