...
export async function POST(req: NextRequest) {
  ...
  // We prefer the tenantId passed by the UI, but fall back to the first link.
  let tenantId = "";
  try {
  ...
  if (!tenantId) {
    return NextResponse.json(
      {
        ok: false,
        error: "domain_not_linked_to_tenant",
      },
      { status: 400 }
    );
  }

  const job = await enqueueJob("mailstack:dkim-rotate", { tenantId, domainId: d.id, domainName: d.name });
