...
export async function GET(req: NextRequest) {
  ...
  // Snoozed leads are hidden by default. Options: hide | include | only
  const snoozed = norm(url.searchParams.get("snoozed") || "hide");
  ...
  // Snooze filter (default: hide snoozed until in the future)
  const now = new Date();
  ...
  if (q) {
    // If OR already exists (from snooze), combine into AND.
    const searchOr = [
      { email: { contains: q } },
  ...
  const items = leads.map((l) => {
    ...
    return {
      ...
      snoozeUntil: (l as any).snoozeUntil || null,
      snoozeReason: (l as any).snoozeReason || null,
