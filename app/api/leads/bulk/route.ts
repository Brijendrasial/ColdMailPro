...
export async function POST(req: NextRequest) {
  ...
  if (action === "snooze") {
    const untilRaw = String(body.until || body.snoozeUntil || body.dueAt || "").trim();
    if (!untilRaw) return NextResponse.json({ ok: false, error: "Missing until" }, { status: 400 });
    const until = new Date(untilRaw);
    if (Number.isNaN(until.getTime())) return NextResponse.json({ ok: false, error: "Invalid until" }, { status: 400 });
    const reason = String(body.reason || "").trim() || null;

    await prisma.lead.updateMany({ where: { id: { in: foundIds }, workspaceId: s.wid }, data: { snoozeUntil: until, snoozeReason: reason } as any });
    await Promise.all(
      foundIds.map((leadId) =>
        logLeadActivity({
          workspaceId: s.wid,
          ...
          actorUserId: s.uid || null,
          type: "snooze",
          text: `Snoozed until ${until.toISOString().slice(0, 10)}`,
  ...
  if (action === "unsnooze") {
    await prisma.lead.updateMany({ where: { id: { in: foundIds }, workspaceId: s.wid }, data: { snoozeUntil: null, snoozeReason: null } as any });
    await Promise.all(
      foundIds.map((leadId) =>
        logLeadActivity({
          workspaceId: s.wid,
