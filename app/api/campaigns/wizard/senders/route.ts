...
export async function POST(req: NextRequest) {
  ...
  const mailboxMinIdleMinutes = Math.max(0, Math.floor(Number(body?.mailboxMinIdleMinutes || 0)));
  ...
  await prisma.campaign.updateMany({
    ...
    data: { mailboxStrategy, mailboxMinIdleMinutes, mailboxPoolId: senderMode === "pool" ? poolIdToSet : null, setupStep: 1, setupCompleted: false },
  });
