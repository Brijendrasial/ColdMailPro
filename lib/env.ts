...
const schema = z.object({
  ...
  WARMUP_MIN_GAP_MINUTES: z.coerce.number().int().nonnegative().default(15),
  WARMUP_THREAD_REUSE_RATE: z.coerce.number().min(0).max(1).default(0.50),
  WARMUP_THREAD_MAX_MESSAGES: z.coerce.number().int().positive().default(4),
  WARMUP_FOLLOWUP_RATE: z.coerce.number().min(0).max(1).default(0.35),
  WARMUP_FOLLOWUP_MIN_DELAY_MIN: z.coerce.number().int().nonnegative().default(60),
  WARMUP_FOLLOWUP_MAX_DELAY_MIN: z.coerce.number().int().nonnegative().default(240),

  // Auto-safety: if a mailbox's warmup placement spikes to spam, automatically disable warmup for that mailbox.
  // This helps prevent burning a mailbox during a bad IP/domain moment.
  WARMUP_AUTO_DISABLE_ON_SPAM: z.coerce.boolean().default(false),
  WARMUP_SPAM_SPIKE_WINDOW_HOURS: z.coerce.number().int().positive().default(24),
  WARMUP_SPAM_SPIKE_MIN_MESSAGES: z.coerce.number().int().positive().default(6),
  WARMUP_SPAM_SPIKE_RATE: z.coerce.number().min(0).max(1).default(0.50),
  ...
  WARMUP_SEED_ENGAGE: z.coerce.boolean().default(true),
  // Probabilities are 0..1
  WARMUP_SEED_ENGAGE_OPEN_RATE: z.coerce.number().min(0).max(1).default(1.0),
  WARMUP_SEED_ENGAGE_STAR_RATE: z.coerce.number().min(0).max(1).default(1.0),
  ...
  // Internal mailbox warmup engagement (for warmup between your own mailboxes)
  WARMUP_MAILBOX_ENGAGE: z.coerce.boolean().default(true),
  WARMUP_MAILBOX_ENGAGE_OPEN_RATE: z.coerce.number().min(0).max(1).default(1.0),
  WARMUP_MAILBOX_ENGAGE_STAR_RATE: z.coerce.number().min(0).max(1).default(1.0),
  ...
  REPLIES_AI_ENABLED: z.coerce.boolean().default(false),

  // --- Google Calendar integration (Replies → schedule meetings) ---
...
export const env = schema.parse({
  ...
  WARMUP_MIN_GAP_MINUTES: process.env.WARMUP_MIN_GAP_MINUTES,
  WARMUP_THREAD_REUSE_RATE: process.env.WARMUP_THREAD_REUSE_RATE,
  WARMUP_THREAD_MAX_MESSAGES: process.env.WARMUP_THREAD_MAX_MESSAGES,
  WARMUP_FOLLOWUP_RATE: process.env.WARMUP_FOLLOWUP_RATE,
  WARMUP_FOLLOWUP_MIN_DELAY_MIN: process.env.WARMUP_FOLLOWUP_MIN_DELAY_MIN,
  WARMUP_FOLLOWUP_MAX_DELAY_MIN: process.env.WARMUP_FOLLOWUP_MAX_DELAY_MIN,
  WARMUP_AUTO_DISABLE_ON_SPAM: process.env.WARMUP_AUTO_DISABLE_ON_SPAM,
  WARMUP_SPAM_SPIKE_WINDOW_HOURS: process.env.WARMUP_SPAM_SPIKE_WINDOW_HOURS,
  WARMUP_SPAM_SPIKE_MIN_MESSAGES: process.env.WARMUP_SPAM_SPIKE_MIN_MESSAGES,
  WARMUP_SPAM_SPIKE_RATE: process.env.WARMUP_SPAM_SPIKE_RATE,
  ...
  WARMUP_MAILBOX_ENGAGE: process.env.WARMUP_MAILBOX_ENGAGE,
  WARMUP_MAILBOX_ENGAGE_OPEN_RATE: process.env.WARMUP_MAILBOX_ENGAGE_OPEN_RATE,
  WARMUP_MAILBOX_ENGAGE_STAR_RATE: process.env.WARMUP_MAILBOX_ENGAGE_STAR_RATE,
  ...
  REPLIES_AI_ENABLED: process.env.REPLIES_AI_ENABLED,
