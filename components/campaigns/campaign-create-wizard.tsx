...
export default function CampaignCreateWizard({ mailboxes, pools, leads, resumeCampaignId }: Props) {
  ...
  // --- Senders ---
  const [mailboxStrategy, setMailboxStrategy] = useState<"round_robin" | "random" | "weighted" | "least_recent" | "score" | "score_idle">("round_robin");
  const [mailboxMinIdleMinutes, setMailboxMinIdleMinutes] = useState<number>(0);
    ...
    async function run() {
      try {
...

        {
          ...
          const allowed = new Set(["round_robin", "random", "weighted", "least_recent", "score", "score_idle"]);
          setMailboxStrategy(((allowed.has(ms) ? ms : "round_robin")) as any);
          setMailboxMinIdleMinutes(Number(c.mailboxMinIdleMinutes ?? 0));
        }
                ...
                <div className="rounded-2xl border border-slate-200 p-3">
                  ...
                  {mailboxStrategy === "score_idle" ? (
                    <div className="mt-3 grid gap-1">
                      <div className="text-sm opacity-80">Minimum idle minutes</div>
                      <input
                        type="number"
                        min={0}
                        className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-transparent"
                        value={mailboxMinIdleMinutes}
                        onChange={(e) => setMailboxMinIdleMinutes(Number((e.target as HTMLInputElement).value || 0))}
                      />
                      <div className="text-xs opacity-60">Prevents the same mailbox from sending back-to-back (recommended: 2–10).</div>
                    </div>
                  ) : null}
                  ...
                  <div className="text-xs opacity-70 mt-2">
                    {mailboxStrategy === "weighted" ? (
                      ...
                      <>Score+idle chooses the healthiest mailbox but enforces a minimum idle gap.</>
